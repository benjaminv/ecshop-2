...
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		$arr[$row['goods_id']]['sales_volume'] = $row['sales_volume']+$row['sales_volume_base']+$row['volume'];
...
function get_exchange_goods_info($goods_id, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
	...
	if ($row !== false) {
        ...
        $row['volume']=$row['sales_volume']+$row['volume'];
...
');
	...
	if (!empty($specs)) {
		$product_info = get_products_info($goods_id, $arr, $region_id, $area_id, $area_city);
	}
	...
	$attr_list = array();
	$sql = 'SELECT a.attr_name, g.attr_value ' . 'FROM ' . $ecs->table('goods_attr') . ' AS g, ' . $ecs->table('attribute') . ' AS a ' . 'WHERE g.attr_id = a.attr_id ' . 'AND g.goods_attr_id ' . db_create_in($arr) . ' ORDER BY a.sort_order, a.attr_id, g.goods_attr_id';
	$res = $db->query($sql);
