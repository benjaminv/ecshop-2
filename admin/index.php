...
');
	...
	if (0 < $adminru['ru_id']) {
		$where_og .= ' AND oi.ru_id = ' . $adminru['ru_id'];
		...
		if (isset($file['error']) && $file['error'] == 0 || !isset($file['error']) && $file['tmp_name'] != 'none') {
			if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
				...
				if (move_upload_file($file['tmp_name'], $file_name)) {
					...
					$sql = ' SELECT count(*) FROM ' . $GLOBALS['ecs']->table('seller_qrcode') . ' WHERE ru_id=\'' . $adminru['ru_id'] . '\'';
					$qrcode_count = $GLOBALS['db']->getOne($sql);

					if (0 < $qrcode_count) {
						if (!empty($qrcode_thumb)) {
							$qrcode_other = array('qrcode_thumb' => $qrcode_thumb);
							$db->autoExecute($ecs->table('seller_qrcode'), $qrcode_other, 'UPDATE', 'ru_id = \'' . $adminru['ru_id'] . '\'');
						}
					}
					else {
						$qrcode_other = array('ru_id' => $adminru['ru_id'], 'qrcode_thumb' => $qrcode_thumb);
						$db->autoExecute($ecs->table('seller_qrcode'), $qrcode_other, 'INSERT');
					}
				}
				else {
	...
	if (admin_priv('complaint', '', false)) {
		$sql = ' SELECT COUNT(*) FROM' . $ecs->table('complaint') . ' AS c ' . 'WHERE c.complaint_state != 4 AND c.ru_id = \'' . $adminru['ru_id'] . '\'  ';
	...
	if (admin_priv('seller_account', '', false)) {
		...
		$sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . ' WHERE bill_apply = 1 AND chargeoff_status = 1';
	...
	if ($is_check_bill) {
		...
		foreach ($seller_list as $key => $row) {
			...
			if ($row['cycle'] == 7) {
				...
				if (empty($day_array)) {
					$sql = 'SELECT MAX(end_time) FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . ' WHERE seller_id = \'' . $row['seller_id'] . '\' AND bill_cycle = \'' . $row['cycle'] . '\' AND repair_order = 0 LIMIT 1';
			...
			if ($day_array) {
				foreach ($day_array as $keys => $rows) {
					...
					$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . ' WHERE seller_id = \'' . $row['seller_id'] . '\' AND bill_cycle = \'' . $row['cycle'] . '\' AND repair_order = 0' . (' AND start_time >= \'' . $last_year_start . '\' AND end_time <= \'' . $last_year_end . '\'');
