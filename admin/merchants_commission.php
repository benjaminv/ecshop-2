...
function merchants_order_list($page = 0)
{
	...
	for ($i = 0; $i < $count; $i++) {
		...
		if (0 < $row[$i]['chargeoff_status']) {
			...
			$sql = 'SELECT bill_sn, commission_model, proportion, chargeoff_status FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . (' WHERE id = \'' . $bill_id . '\' LIMIT 1');
		...
		if ($account_log && $account_log['percent_value']) {
			...
			$row[$i]['bill_sn'] = $bill['bill_sn'];
			$row[$i]['chargeoff_status'] = $bill['chargeoff_status'];
			if ($row[$i]['chargeoff_status'] == 0 && 0 < $bill['chargeoff_status']) {
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') . ' SET chargeoff_status = \'' . $bill['chargeoff_status'] . '\' WHERE order_id = \'' . $row[$i]['order_id'] . '\'';
...
if ($_REQUEST['act'] == 'list') {
	...
	if ($type == 'all') {
		...
		if (0 < $store_num) {
			$sql = ' SELECT SUM(' . order_commission_field('o.') . ') AS total_amount ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o WHERE o.ru_id ' . db_create_in($seller);
	...
	$arr = array();
	$admin_commission = array();
	$admin_commission_is_settlement = 0;
	$admin_commission_no_settlement = 0;
	...
	if ($seller) {
		for ($i = 0; $i < count($seller); $i++) {
			if (file_exists(MOBILE_DRP)) {
				...
				$arr[$i]['is_settlement'] = isset($is_settlement['all']) ? $is_settlement['all'] : 0;
				$arr[$i]['no_settlement'] = isset($no_settlement['all']) ? $no_settlement['all'] : 0;
				$arr[$i]['commission_is_settlement'] = isset($is_settlement['all_price']) ? $is_settlement['all_price'] : 0;
				$arr[$i]['commission_no_settlement'] = isset($no_settlement['all_price']) ? $no_settlement['all_price'] : 0;
			}
			else {
				$is_settlement = merchants_is_settlement($seller[$i], 1);
				$no_settlement = merchants_is_settlement($seller[$i], 0);
				$is_settlement = $is_settlement ? $is_settlement : 0;
				$no_settlement = $no_settlement ? $no_settlement : 0;
				$arr[$i]['commission_is_settlement'] = $is_settlement;
				$arr[$i]['commission_no_settlement'] = $no_settlement;
				$arr[$i]['is_settlement'] = price_format($is_settlement);
				$arr[$i]['no_settlement'] = price_format($no_settlement);
			}

			$arr[$i]['user_id'] = $seller[$i];
			$admin_commission_is_settlement += $arr[$i]['commission_is_settlement'];
			$admin_commission_no_settlement += $arr[$i]['commission_no_settlement'];
		}
	}

	$admin_commission['is_settlement'] = $admin_commission_is_settlement;
	$admin_commission['no_settlement'] = $admin_commission_no_settlement;
	...
	if ($type == 'all') {
		...
		$admin_all['no_settlement'] = empty($admin_commission['no_settlement']) ? '0.00' : $admin_commission['no_settlement'];
...
');
	...
	$smarty->assign('bill_apply', $bill_apply);
	...
	$sql = 'SELECT order_id FROM ' . $ecs->table('seller_bill_order') . (' WHERE bill_id = \'' . $id . '\'');
	$order_list = $db->getCol($sql);
	$sql = 'DELETE FROM ' . $ecs->table('seller_commission_bill') . (' WHERE id = \'' . $id . '\'');
	$db->query($sql);
	$sql = 'UPDATE ' . $ecs->table('seller_bill_order') . (' SET bill_id = 0, chargeoff_status = 0, repair = 0 WHERE bill_id = \'' . $id . '\' AND chargeoff_status < 2');
	$db->query($sql);

	if ($order_list) {
		$sql = 'UPDATE ' . $ecs->table('order_info') . ' SET chargeoff_status = 0 WHERE order_id ' . db_create_in($order_list);
		$db->query($sql);
		$sql = 'UPDATE ' . $ecs->table('order_return') . ' SET chargeoff_status = 0 WHERE order_id ' . db_create_in($order_list);
		$db->query($sql);
	}
...
');
	...
	if ($type == 'bill_frozen' || $type == 'bill_unfreeze') {
		...
		if ($check_status == 1 && $bill['bill_apply'] == 1 && $bill['chargeoff_status'] <= 1) {
			$other = array('chargeoff_status' => $chargeoff_status, 'check_status' => $check_status, 'check_time' => gmtime(), 'settleaccounts_time' => $settleaccounts_time);
		...
		else if (0 < $check_status) {
			if ($bill['bill_apply'] == 1 && $bill['chargeoff_status'] <= 1) {
				if (0 < $check_status) {
					$other = array('check_status' => $check_status, 'reject_note' => $reject_note);
					$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_commission_bill'), $other, 'UPDATE', 'id = \'' . $bill_id . '\'');
				}
...
else if ($_REQUEST['act'] == 'commission_chang') {
	...
	$record_count = count($seller_list);
	$smarty->assign('record_count', $record_count);
	$smarty->assign('page', 1);
	assign_query_info();
	$smarty->display('commission_chang.dwt');
}
else if ($_REQUEST['act'] == 'chang_commission_list') {
	include_once ROOT_PATH . 'includes/cls_json.php';
	$json = new JSON();
	$page = !empty($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
	$page_size = isset($_REQUEST['page_size']) ? intval($_REQUEST['page_size']) : 1;
	$seller_list = read_static_cache('seller_list', '/data/sc_file/');
	$seller_list = $ecs->page_array($page_size, $page, $seller_list);
	$result['list'] = $seller_list['list'][0];

	if ($result['list']) {
		$result['list']['shop_name'] = get_shop_name($result['list']['seller_id'], 1);
		$sql = 'SELECT * FROM ' . $ecs->table('merchants_server') . ' WHERE user_id = \'' . $result['list']['seller_id'] . '\'';
		$server = $db->getRow($sql);
		$cycle = !empty($server) ? $server['cycle'] : 0;
		$sql = 'SELECT GROUP_CONCAT(id) as id FROM ' . $ecs->table('seller_commission_bill') . ' WHERE seller_id = \'' . $result['list']['seller_id'] . ('\' AND bill_cycle <> ' . $cycle . ' AND chargeoff_status < 2 ORDER BY id DESC');
		$bill_list = $db->getOne($sql);
		$result['list']['bill_list'] = $bill_list;

		if ($bill_list) {
			$sql = 'SELECT GROUP_CONCAT(order_id) as order_id FROM ' . $ecs->table('seller_bill_order') . ' WHERE bill_id ' . db_create_in($bill_list);
			$order_list = $db->getOne($sql);
			$sql = 'UPDATE ' . $ecs->table('seller_bill_order') . ' SET bill_id = 0, chargeoff_status = 0 WHERE bill_id ' . db_create_in($bill_list) . ' AND chargeoff_status < 2';
			$db->query($sql);

			if ($order_list) {
				$sql = 'UPDATE ' . $ecs->table('order_info') . ' SET chargeoff_status = 0 WHERE order_id ' . db_create_in($order_list) . ' AND chargeoff_status < 2';
				$db->query($sql);
				$sql = 'UPDATE ' . $ecs->table('order_return') . ' SET chargeoff_status = 0 WHERE order_id ' . db_create_in($order_list) . ' AND chargeoff_status < 2';
				$db->query($sql);
			}

			$sql = 'DELETE FROM ' . $ecs->table('seller_commission_bill') . ' WHERE id ' . db_create_in($bill_list);
			$db->query($sql);
			$result['list']['handle'] = '处理';
		}
		else {
			$result['list']['handle'] = '不处理';
		}
	}

	$result['page'] = $seller_list['filter']['page'] + 1;
	$result['page_size'] = $seller_list['filter']['page_size'];
	$result['record_count'] = $seller_list['filter']['record_count'];
	$result['page_count'] = $seller_list['filter']['page_count'];
	$result['is_stop'] = 1;

	if ($seller_list['filter']['page_count'] < $page) {
		$result['is_stop'] = 0;
	}
	else {
		$result['filter_page'] = $seller_list['filter']['page'];
	}

	exit($json->encode($result));
}
else if ($_REQUEST['act'] == 'replacement_order') {
	admin_priv('order_view');
	$smarty->assign('ur_here', '更新订单商家ID');
	...
	if (!$order_list) {
		...
		if ($bill_list) {
			$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('seller_bill_order') . (' WHERE seller_id = \'' . $seller_id . '\'') . ' AND bill_id = 0 AND chargeoff_status = 0 ' . ' AND order_status = \'' . OS_SPLITED . '\' AND shipping_status = \'' . SS_RECEIVED . '\' AND pay_status = \'' . PS_PAYED . '\' ' . ' AND confirm_take_time <= \'' . $bill_list['end_time'] . '\' ORDER BY order_id DESC';
			$order_list = $GLOBALS['db']->getAll($sql);
		}

		write_static_cache('commission_replacement_order' . $seller_id, $order_list, '/data/sc_file/');
	}
	...
	if (0 < $record_count) {
		$first = $order_list[0];
		$last = $order_list[$record_count - 1];
		$last_year_start = $last['confirm_take_time'];
		$last_year_end = local_strtotime(local_date('Y-m-d', gmtime()));
		$sql = 'SELECT IFNULL(s.cycle, 0) AS cycle, p.percent_value, s.day_number, s.bill_time FROM ' . $GLOBALS['ecs']->table('merchants_server') . ' AS s ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_percent') . ' AS p ON s.suppliers_percent = p.percent_id' . (' WHERE s.user_id = \'' . $seller_id . '\'');
		$seller_info = $GLOBALS['db']->getRow($sql);
		$sql = 'SELECT id FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . (' where seller_id = \'' . $seller_id . '\' AND start_time = \'' . $last_year_start . '\' AND end_time = \'' . $last_year_end . '\'');
		$bill_id = $GLOBALS['db']->getOne($sql);

		if (!$bill_id) {
			$bill_sn = get_order_sn();
			$other = array('seller_id' => $seller_id, 'bill_sn' => $bill_sn, 'proportion' => $seller_info['percent_value'], 'start_time' => $last_year_start, 'end_time' => $last_year_end, 'bill_cycle' => $seller_info['cycle'], 'operator' => $_SESSION['admin_name'], 'repair_order' => 1);
	...
	assign_query_info();
	$smarty->display('commission_replacement.dwt');
}
else if ($_REQUEST['act'] == 'chang_replacement_order') {
	include_once ROOT_PATH . 'includes/cls_json.php';
	$json = new JSON();
	...
	if ($order_list === false) {
		...
		if ($bill_list) {
			$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('seller_bill_order') . (' WHERE seller_id = \'' . $seller_id . '\'') . ' AND bill_id = 0 AND chargeoff_status = 0 ' . ' AND order_status = \'' . OS_SPLITED . '\' AND shipping_status = \'' . SS_RECEIVED . '\' AND pay_status = \'' . PS_PAYED . '\' ' . ' AND confirm_take_time < \'' . $bill_list['start_time'] . '\' ORDER BY order_id DESC';
			$order_list = $GLOBALS['db']->getAll($sql);
		}
	...
	$sql = 'SELECT s.commission_model, p.percent_value FROM ' . $GLOBALS['ecs']->table('merchants_server') . ' AS s ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_percent') . ' AS p ON s.suppliers_percent = p.percent_id' . (' WHERE user_id = \'' . $seller_id . '\'');
	$server = $GLOBALS['db']->getRow($sql);
	$order_list = $ecs->page_array($page_size, $page, $order_list);
	$result['list'] = $order_list['list'][0];

	if ($result['list']) {
		$total_fee = $result['list']['goods_amount'] + $result['list']['tax'] + $result['list']['shipping_fee'] + $result['list']['insure_fee'] + $result['list']['pay_fee'] + $result['list']['pack_fee'] + $result['list']['card_fee'];
		$commission_total_fee = $total_fee - $result['list']['discount'] - $result['list']['coupons'] - $result['list']['integral_money'] - $result['list']['bonus'];
		$order_amount = $total_fee - $result['list']['discount'];
		$shipping_amount = $result['list']['shipping_fee'];
		$return_amount = $result['list']['return_amount'];
		$return_shippingfee = $result['list']['return_shippingfee'];
		$drp_money = $result['list']['drp_money'];
		$commission_model = $server ? $server['commission_model'] : 0;
		$sql = 'UPDATE ' . $ecs->table('seller_commission_bill') . ' SET order_amount = order_amount + ' . $order_amount . ', shipping_amount = shipping_amount + ' . $shipping_amount . (', return_amount = return_amount + ' . $return_amount) . (', return_shippingfee = return_shippingfee + ' . $return_shippingfee) . (', drp_money = drp_money + ' . $drp_money) . ', commission_model = ' . $commission_model . (' WHERE id = \'' . $bill_id . '\'');
		$db->query($sql);
		$sql = 'UPDATE ' . $ecs->table('seller_bill_order') . ' SET bill_id = \'' . $bill_id . '\', chargeoff_status = 1, repair = 1 WHERE order_id = \'' . $result['list']['order_id'] . '\'';
		$db->query($sql);
		$sql = 'UPDATE ' . $ecs->table('order_info') . ' SET chargeoff_status = 1 WHERE order_id = \'' . $result['list']['order_id'] . '\'';
		$db->query($sql);
		$sql = 'UPDATE ' . $ecs->table('order_return') . ' SET chargeoff_status = 1 WHERE order_id = \'' . $result['list']['order_id'] . '\'';
		$db->query($sql);
	}
	...
	if ($order_list['filter']['page_count'] < $page) {
		$bill = array('seller_id' => $seller_id, 'bill_id' => $bill_id, 'commission_model' => $server['commission_model'], 'proportion' => $server['percent_value']);
		$detail = bill_detail_list(1, $bill);
		$other['chargeoff_status'] = 1;
		$other['chargeoff_time'] = gmtime();
		$other['gain_commission'] = $detail['bill_gain_commission'];
		$other['should_amount'] = $detail['bill_should_amount'];
		$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_commission_bill'), $other, 'UPDATE', 'id = \'' . $bill_id . '\'');
		$result['is_stop'] = 0;
