...
function get_exchange_goodslist($ru_id)
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT eg.* , g.goods_name, g.sales_volume,g.sales_volume_base ' . 'FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS eg ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = eg.goods_id ' . ('WHERE ' . $where . ' ORDER BY ') . $filter['sort_by'] . ' ' . $filter['sort_order'];
...
if ($_REQUEST['act'] == 'update') {
	...
	if ($sales_volume_base == '') {
		make_json_error('不能为空');
	}else{
		$sql = 'UPDATE ' . $ecs->table('goods') . ' SET sales_volume_base ='.$sales_volume_base.' WHERE goods_id =' . $goods_id ;
		$db->query($sql);
		clear_cache_files();
