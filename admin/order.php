...
function order_list($page = 0)
{
	...
	if ($result === false) {
		...
		if ($filter['seller_list']) {
			...
			$where .= ' AND (o.ru_id = 0 OR (o.is_zc_order > 0)) ';
		}
		...
		$select = '(SELECT bai.is_stages FROM ' . $GLOBALS['ecs']->table('baitiao_log') . ' as bai WHERE o.order_id = bai.order_id) AS is_stages,';
		$sql = 'SELECT ' . $select . ' o.extension_code as oi_extension_code, o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.pay_status, o.order_amount, o.money_paid, o.is_delete,' . 'o.shipping_fee, o.insure_fee, o.pay_fee, o.pack_fee, o.card_fee, o.surplus, o.tax, o.integral_money, o.bonus, o.discount, o.coupons,' . 'o.shipping_time, o.auto_delivery_time, o.consignee, o.address, o.email, o.tel, o.mobile, o.extension_code as o_extension_code, o.extension_id, o.is_zc_order, o.pay_id, ' . 'o.pay_name, o.referer, o.froms, o.user_id, o.chargeoff_status, o.confirm_take_time, o.shipping_id, o.shipping_name, o.goods_amount, ' . '(' . order_amount_field('o.') . ') AS total_fee, (o.goods_amount + o.tax + o.shipping_fee + o.insure_fee + o.pay_fee + o.pack_fee + o.card_fee - o.discount) AS total_fee_order' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . $leftJoin . $where . $status_where . $where_store . $no_main_order . $groupBy . (' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ') . ' LIMIT ' . ($filter['page'] - 1) * $filter['page_size'] . (',' . $filter['page_size']);
		...
		if ($GLOBALS['_CFG']['open_delivery_time'] == 1) {
			if (($value['order_status'] == OS_CONFIRMED || $value['order_status'] == OS_SPLITED) && $value['pay_status'] == PS_PAYED && $value['shipping_status'] == SS_SHIPPED && $value['chargeoff_status'] == 0) {
				...
				if ($delivery_time < $confirm_take_time) {
					$sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') . ' SET order_status = \'' . $value['order_status'] . '\', ' . 'shipping_status = \'' . SS_RECEIVED . '\', pay_status = \'' . $value['pay_status'] . ('\', confirm_take_time = \'' . $confirm_take_time . '\' WHERE order_id = \'') . $value['order_id'] . '\'';
					...
					$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('seller_bill_order') . ' WHERE order_id = \'' . $value['order_id'] . '\'';
					$bill_order_info = $GLOBALS['db']->getOne($sql);
			...
			else if (empty($value['confirm_take_time'])) {
				$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('seller_bill_order') . ' WHERE order_id = \'' . $value['order_id'] . '\'';
				$bill_order_count = $GLOBALS['db']->getOne($sql, true);
				if (0 < $bill_order_count && $value['shipping_status'] == SS_RECEIVED) {
					...
					$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_bill_order'), $log_other, 'UPDATE', 'order_id = \'' . $value['order_id'] . '\'');
				}
			}
		}

		if (0 < $value['chargeoff_status']) {
			$bill = $GLOBALS['db']->getRow(' SELECT scb.id, scb.bill_sn, scb.seller_id, scb.proportion, scb.commission_model FROM ' . $GLOBALS['ecs']->table('seller_bill_order') . ' AS sbo ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('seller_commission_bill') . ' AS scb ON sbo.bill_id = scb.id ' . 'WHERE sbo.order_id = \'' . $value['order_id'] . '\' ');
		}
		else {
			$sql = 'SELECT bill_id, chargeoff_status FROM ' . $GLOBALS['ecs']->table('seller_bill_order') . ' WHERE order_id = \'' . $value['order_id'] . '\' AND chargeoff_status > 0';
			$bill = $GLOBALS['db']->getRow($sql);

			if ($bill) {
				$bill = $GLOBALS['db']->getRow('SELECT * FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . 'WHERE id = \'' . $bill['bill_id'] . '\'');
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') . ' SET chargeoff_status = \'' . $bill['chargeoff_status'] . '\' WHERE order_id = \'' . $value['order_id'] . '\'';
				$GLOBALS['db']->query($sql);
			}
		}
...
function get_is_baitiao_order($order_sn = '')
{
	$is_baitiao_order = 0;

	if ($order_sn) {
		$sql = 'SELECT stages_id FROM ' . $GLOBALS['ecs']->table('stages') . ' WHERE order_sn = \'' . $order_sn . '\'';
...
if ($_REQUEST['act'] == 'order_download') {
	...
	$order['is_baitiao_order'] = get_is_baitiao_order($order['order_sn']);
	...
	if ($GLOBALS['_CFG']['open_delivery_time'] == 1) {
		...
		if (($orderInfo['order_status'] == OS_CONFIRMED || $orderInfo['order_status'] == OS_SPLITED) && $orderInfo['shipping_status'] == SS_SHIPPED && $orderInfo['pay_status'] == PS_PAYED) {
			...
			if ($delivery_time < $confirm_take_time) {
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') . ' SET order_status = \'' . $orderInfo['order_status'] . '\', ' . 'shipping_status = \'' . SS_RECEIVED . '\', pay_status = \'' . $orderInfo['pay_status'] . ('\', confirm_take_time = \'' . $confirm_take_time . '\' WHERE order_id = \'') . $order['order_id'] . '\'';
				$GLOBALS['db']->query($sql);
				$note = $GLOBALS['_LANG']['self_motion_goods'];
		...
		else if (empty($order['confirm_take_time'])) {
			$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('seller_bill_order') . ' WHERE order_id = \'' . $order['order_id'] . '\'';
			$bill_order_count = $GLOBALS['db']->getOne($sql, true);
			if (0 < $bill_order_count && $order['shipping_status'] == SS_RECEIVED) {
				...
				$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_bill_order'), $log_other, 'UPDATE', 'order_id = \'' . $order['order_id'] . '\'');
			}
		}
	}
	...
	while ($row = $db->fetchRow($res)) {
		...
		$goods_where = ' AND d.delivery_id = any(SELECT dg.delivery_id FROM ' . $GLOBALS['ecs']->table('delivery_goods') . ' AS dg WHERE dg.goods_id = \'' . $row['goods_id'] . '\' AND goods_attr = \'' . $row['goods_attr'] . '\')';
...
');
	...
	if ($_SESSION['is_ajax_detection'] == 1) {
		...
		if ($result['order']) {
			...
			$GLOBALS['db']->query('UPDATE ' . $GLOBALS['ecs']->table('order_info') . (' SET confirm_take_time = \'' . $confirm_take_time . '\', ') . 'order_status = \'' . $result['order']['order_status'] . '\', ' . 'shipping_status = \'' . SS_RECEIVED . '\', pay_status = \'' . $result['order']['pay_status'] . '\'' . ' WHERE order_id = \'' . $result['order']['order_id'] . '\'');
		...
		if ('confirm' == $operation) {
			...
			if ($order['pay_status'] != PS_UNPAYED) {
				...
				if (!empty($_REQUEST['action_note'])) {
					...
					if ($order['chargeoff_status'] == 1) {
						$sql = 'SELECT bill_id FROM ' . $GLOBALS['ecs']->table('seller_bill_order') . (' WHERE order_id = \'' . $order_id . '\'');
						$bill_id = $GLOBALS['db']->getOne($sql, true);
						$sql = 'SELECT b.*, s.commission_model, s.bill_freeze_day FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . ' AS b ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_server') . ' AS s ON b.seller_id = s.user_id AND b.bill_cycle = s.cycle ' . (' WHERE id = \'' . $bill_id . '\' LIMIT 1');
						$bill_info = $GLOBALS['db']->getRow($sql);

						if ($bill_info) {
							$bill_detail = get_bill_amount_detail($bill_info['id'], $bill_info['seller_id'], $bill_info['proportion'], $bill_info['start_time'], $bill_info['end_time'], $bill_info['chargeoff_status'], $bill_info['commission_model']);
							...
							$bill_other['chargeoff_time'] = gmtime();
							$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_commission_bill'), $bill_other, 'UPDATE', 'id = \'' . $bill_info['id'] . '\'');
						}
					}
				}
			}
...
');
	...
	else if ($_REQUEST['act'] == 'chang_order') {
		admin_priv('order_view');
		$smarty->assign('ur_here', '更新订单商家ID');
		$order_list = read_static_cache('chang_order_list', '/data/sc_file/');

		if ($order_list === false) {
			$no_main_order = ' and (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0 ';
			$sql = 'SELECT o.order_id, o.order_sn FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' WHERE (SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og WHERE o.order_id = og.order_id AND og.ru_id > 0) > 0 AND o.ru_id = 0' . $no_main_order;
			$order_list = $GLOBALS['db']->getAll($sql);
			write_static_cache('chang_order_list', $order_list, '/data/sc_file/');
		}

		$record_count = count($order_list);
		$smarty->assign('record_count', $record_count);
		$smarty->assign('page', 1);
		assign_query_info();
		$smarty->display('order_chang.dwt');
	}
	else if ($_REQUEST['act'] == 'chang_order_list') {
		include_once ROOT_PATH . 'includes/cls_json.php';
		$json = new JSON();
		...
		if ($order_list === false) {
			$no_main_order = ' and (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0 ';
			$sql = 'SELECT o.order_id, o.order_sn FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' WHERE (SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og WHERE o.order_id = og.order_id AND og.ru_id > 0) > 0 AND ru_id' . $no_main_order;
			$order_list = $GLOBALS['db']->getAll($sql);
			write_static_cache('chang_order_list', $order_list, '/data/sc_file/');
		}
		...
		if ($order_list) {
			foreach ($order_list as $key => $row) {
				$sql = 'SELECT ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' WHERE order_id = \'' . $row['order_id'] . '\'';
				$ru_id = $GLOBALS['db']->getOne($sql, true);
		...
		if ($result['list']) {
			$sql = 'UPDATE ' . $ecs->table('order_info') . ' SET ru_id = \'' . $result['list']['ru_id'] . '\' WHERE order_id = \'' . $result['list']['order_id'] . '\'';
			$db->query($sql);
		}
