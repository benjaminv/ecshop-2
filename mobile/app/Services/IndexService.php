...
class IndexService
{
	...
	{
		$res = $this->shopRepository->getPositions('wxapp', 10);
		$ads = array();
	...
	{
		$res = $this->shopRepository->getArticle();
		$rootPath = app('request')->root();
		$rootPath = dirname(dirname($rootPath)) . '/';

		foreach ($res as $k => $v) {
			$res[$k]['content'] = str_replace(array('src="/images/upload', 'src="images/upload'), 'src="' . $rootPath . '/images/upload', $v['content']);
		}
