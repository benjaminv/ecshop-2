...
', str_replace('
', '', $this->shopConfigRepository->getShopConfigByCode('invoice_content')));
	...
	{
		$cartGoodsList = $this->cartRepository->getGoodsInCartByUser($userId, $flow_type, true);
		$list = array();
	...
	public function submitOrder($args)
	{
		...
		$goodsNum = $this->cartRepository->goodsNumInCartChecked($userId, $flow_type);
		...
		if ($this->shopConfigRepository->getShopConfigByCode('use_storage') == 1 && $this->shopConfigRepository->getShopConfigByCode('stock_dec_time') == 1) {
			$cart_goods = $this->cartRepository->getGoodsInCartByUser($userId, $flow_type, true);
		...
		$is_distribution = 0;
		if ($order['extension_code'] != 'bargain_buy' && $order['extension_code'] != 'team_buy') {
			$drp_affiliate = $this->drpRepository->drpType('drp_affiliate');
			$drp_affiliate = unserialize($drp_affiliate['value']);
			if (isset($drp_affiliate['on']) && $drp_affiliate['on'] == 1) {
				$isdistribution = $this->drpRepository->drpType('isdistribution');
				$isdistribution = $isdistribution['value'];
				...
				if ($isdistribution == 1) {
					...
					$shopIinfo = $this->drpRepository->drpShopInfo($userId);
		...
		foreach ($cartGoods as $v) {
			foreach ($v['goods'] as $gv) {
				...
				$gv['is_distribution'] = $gv['is_distribution'];
				$gv['isdistribution'] = $is_distribution;
				$newGoodsList[] = $gv;
			}
		}
		...
		$res = \App\Models\OrderGoods::where('order_id', $new_order_id)->get();
		$res = $res ? $res->toArray() : array();
		$shopList = array();

		if ($res) {
			foreach ($res as $key => $row) {
				$shopList[] = isset($row['ru_id']) ? $row['ru_id'] : 0;
			}
		}

		$ru_number = count($shopList);
		if ($flow_type != CART_BARGAIN_GOODS && $flow_type != CART_TEAM_GOODS && 1 < $ru_number) {
			...
			foreach ($cart_goods['goods_list'] as $k => $v) {
				foreach ($v['goods'] as $key => $val) {
					$cart_goods['goods_list'][$k]['goods'][$key]['isdistribution'] = $is_distribution;
				}
			}
			...
			\App\Models\OrderInfo::where('order_id', $new_order_id)->update(array('ru_id' => $shopList[0]));
		}

		$this->drpService->addDrpLog();
		return $order_id;
	}
