...
function add_drp_log()
{
	...
	$sql = 'SELECT o.order_id FROM {pre}order_info  ' . ' o' . ' LEFT JOIN' . $GLOBALS['ecs']->table('users') . ' u ON o.user_id = u.user_id' . ' WHERE o.user_id > 0 AND (o.drp_is_separate = 0 OR o.drp_is_separate > 0) ' . (' ' . $sqladd . ' ') . (' and o.order_id > ' . $last_oid);
	...
	if ($up_oid) {
		foreach ($up_oid as $kk => $vv) {
			...
			for ($i = 0; $i < $num; $i++) {
				...
				$per = $coco;
				$is_distribution = dao('drp_config')->field('value')->where(array('code' => 'isdistribution'))->find();
				if (isset($is_distribution['value']) && $is_distribution['value'] == 1) {
					$user = $GLOBALS['db']->getRow('SELECT o.drp_parent_id as user_id, u.id as drp_shop_id FROM ' . $GLOBALS['ecs']->table('users') . ' o' . ' LEFT JOIN' . $GLOBALS['ecs']->table('drp_shop') . ' u ON o.drp_parent_id = u.user_id' . (' WHERE o.user_id = \'' . $row['user_id'] . '\''));
				}
				else if ($i == 0) {
					$user = $GLOBALS['db']->getRow('SELECT o.user_id, u.id as drp_shop_id FROM ' . $GLOBALS['ecs']->table('users') . ' o' . ' LEFT JOIN' . $GLOBALS['ecs']->table('drp_shop') . ' u ON o.user_id = u.user_id' . (' WHERE u.user_id = \'' . $row['user_id'] . '\''));
				...
				$user_ino = dao('users')->field('user_name')->where(array('user_id' => $row['user_id']))->find();
				$row['user_name'] = $user_ino['user_name'];
				if (!isset($user['drp_shop_id']) && !$user['drp_shop_id']) {
					break;
				}
				...
				if (0 < $setmoney && 0 < $row['user_id']) {
					writeDrpLog($vv['order_id'], $row['user_id'], $row['user_name'], $setmoney, $setpoint, $i, $is, 0);
				}
			}
		}
	}
...
function writeDrpLog($oid, $uid, $username, $money, $point, $i, $is, $separate_by)
{
	$time = gmtime();
...
function drp_rank_info($user_id = 0)
{
	...
	if (0 < $drp_info['credit_id']) {
		...
		$sql = 'SELECT sum(money) as totals FROM ' . $GLOBALS['ecs']->table('order_goods') . ' o' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('drp_affiliate_log') . ' a ON o.order_id = a.order_id' . ' WHERE a.separate_type != -1 and a.user_id = ' . $user_id;
