...
{
	$sql = 'select oi.ru_id from ' . $GLOBALS['ecs']->table('order_info') . ' as oi ' . (' where oi.order_id = \'' . $order_id . '\'');
...
{
	$sql = 'select province, city, meiqia, kf_type, kf_ww, kf_qq, shop_name from ' . $GLOBALS['ecs']->table('seller_shopinfo') . (' where ru_id=\'' . $ru_id . '\'');
	return $GLOBALS['db']->getRow($sql);
}
