...
class GoodsRepository
{
	...
	public function findBy($field, $value, $page = 1, $size = 10, $columns = array('*'), $keywords = '', $sortKey = '', $sortVal = '', $proprietary = 3, $price_min = 0, $price_max = 0, $brand = '', $province_id = 0, $city_id = 0, $county_id = 0, $fil_key)
	{
		...
		$res = $goods->where('is_on_sale', 1)->where('is_delete', 0)->where('review_status', '>', 2)->offset($begin)->limit($size)->get()->toArray();
		return $res;
	}
...
', '<br />', $v['attr_value']);
	...
	{
		$product = \App\Models\Products::select('product_id as id', 'product_sn')->where('goods_id', $goodsId);

		if (!empty($goodsAttr)) {
			$attr_arr = explode('|', $goodsAttr);

			if ($attr_arr) {
				foreach ($attr_arr as $key => $val) {
					$product = $product->whereRaw('FIND_IN_SET(\'' . $val . '\', REPLACE(goods_attr, \'|\', \',\'))');
				}
			}
		}
	...
	public function getProductsAttrNumber($goods_id, $attr_id, $warehouse_id, $area_id, $model_attr = 0, $store_id = 0)
	{
		...
		if (0 < $store_id) {
			$product_number = \App\Models\StoreProducts::select('product_number')->where('goods_id', $goods_id)->where('store_id', $store_id);
		}
		else if ($model_attr == 1) {
			$product_number = \App\Models\ProductsWarehouse::select('product_number')->where('goods_id', $goods_id)->where('warehouse_id', $warehouse_id);
		}
		else if ($model_attr == 2) {
			$product_number = \App\Models\ProductsArea::select('product_number')->where('goods_id', $goods_id)->where('area_id', $area_id);
		}
		else {
			$product_number = \App\Models\Products::select('product_number', 'cloud_product_id', 'inventoryid')->where('goods_id', $goods_id);
		}

		if (!empty($attr_id)) {
			if ($attr_arr) {
				foreach ($attr_arr as $key => $val) {
					$product_number = $product_number->whereRaw('FIND_IN_SET(\'' . $val . '\', REPLACE(goods_attr, \'|\', \',\'))');
				}
			}
		}

		$product_number = $product_number->first();
	...
	public function getProductsAttrPrice($goods_id, $attr_id, $warehouse_id, $area_id, $model_attr = 0)
	{
		...
		if ($this->shopConfigRepository->getShopConfigByCode('goods_attr_price') == 1) {
			if ($model_attr == 1) {
				$product_price = \App\Models\ProductsWarehouse::select('product_price', 'product_promote_price', 'product_market_price')->where('goods_id', $goods_id)->where('warehouse_id', $warehouse_id);
			}
			else if ($model_attr == 2) {
				$product_price = \App\Models\ProductsArea::select('product_price', 'product_promote_price', 'product_market_price')->where('goods_id', $goods_id)->where('area_id', $area_id);
			}
			else {
				$product_price = \App\Models\Products::select('product_price', 'product_promote_price', 'product_market_price')->where('goods_id', $goods_id);
			}
	...
	public function goodsHistory($historylist, $page = 1, $size = 10)
	{
		...
		foreach ($historylist as $key => $val) {
			$goods[$key] = \App\Models\Goods::select('goods_id', 'goods_name', 'goods_number', 'market_price', 'shop_price', 'goods_thumb', 'goods_img')->where('goods_id', $val)->where('review_status', '>', 2)->where('is_on_sale', 1)->where('is_delete', 0)->offset($begin)->limit($size)->get()->toArray();
