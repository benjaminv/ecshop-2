...
class ShopRepository
{
	...
	public function getStore()
	{
		...
		foreach ($store as $key => $val) {
			...
			$store[$key]['goods'] = \App\Models\Goods::select('goods_id', 'goods_name', 'goods_thumb')->where('user_id', $val['user_id'])->where('is_on_sale', '1')->where('review_status', '>', 2)->where('is_alone_sale', '1')->limit(3)->orderBy('sort_order', 'ASC')->get()->toArray();
	...
	{
		$res = \App\Models\Article::select('title', 'content', 'article_id')->where('is_open', 1)->where('cat_id', '>', 0)->limit(10)->orderBy('add_time', 'desc')->get()->toArray();
		return $res;
	}
}
