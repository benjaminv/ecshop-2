...
class ApiSellerController extends \App\Modules\Admin\Controllers\EditorSellerController
{
	...
	public function actionAddCollect()
	{
		...
		if (!empty($shopid) && 0 < $_SESSION['user_id']) {
			...
			if (0 < count($status)) {
				...
				$this->db->query('INSERT INTO {pre}collect_store (user_id, ru_id, add_time, is_attention) VALUES (' . $_SESSION['user_id'] . (',\'' . $shopid . '\',') . gmtime() . ',1)');
	...
	public function actionMakeGallery()
	{
		if (IS_POST) {
			$ru_id = input('ruid');
			$time = gmtime();
			$keep = array('ru_id' => $ru_id, 'album_mame' => '商家店铺相册', 'sort_order' => 50, 'add_time' => $time);
	...
	public function actionSearch()
	{
		if (IS_POST) {
			...
			$where = 'g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.user_id = ' . $user_id . ' AND msi.shop_close = 1 ';
