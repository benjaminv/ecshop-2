...
abstract class FrontendController extends FoundationController
{
	...
	protected function kefu($goods_id = 0, $shop_id = 0)
	{
		$config = load_ecsconfig();
		$shop_config = get_seller_shopinfo($shop_id);
		$qq = $shop_config['kf_qq'] ? explode('
', $shop_config['kf_qq']) : '';
		$ww = $shop_config['kf_ww'] ? explode('
', $shop_config['kf_ww']) : '';

		if (isset($qq[0])) {
			$qq = trim(end(explode('|', $qq[0])));
		}

		if (isset($ww[0])) {
			$ww = trim(end(explode('|', $ww[0])));
		}

		if ($config['customer_service'] == 0) {
			return url('chat/index/index', array('goods_id' => $goods_id, 'ru_id' => $shop_id), false, true);
		}
		else if (isset($shop_config['kf_touid'])) {
			return url('chat/yunwang/index', array('goods_id' => $goods_id, 'ru_id' => $shop_id), false, true);
		}
		else if (!empty($shop_config['meiqia'])) {
			return 'https://e-' . trim($shop_config['meiqia']) . '.chatnow.meiqia.com/dist/standalone.html';
		}
		else if ($shop_config['kf_type'] == 0) {
			return 'https://wpa.qq.com/msgrd?v=3&uin=' . $qq . '&site=qq&menu=yes';
		}
		else if ($shop_config['kf_type'] == 1) {
			return 'https://www.taobao.com/webww/ww.php?ver=3&touid=' . $ww . '&siteid=cntaobao&status=1&charset=utf-8';
		}
	}
...
', '
'), '', html_in($share_data['desc'])) : C('shop.shop_desc'), 'link' => !empty($share_data['link']) ? $share_data['link'] : $this->get_current_url(), 'img' => $share_img);
		return $data;
	}
}
