...
if ($action == 'register') {
	...
	$register_article_id = isset($_CFG['register_article_id']) && !empty($_CFG['register_article_id']) ? $_CFG['register_article_id'] : '6';
	$register_url = build_uri('article', array('aid' => $register_article_id), $_LANG['protocol_bind']);
	$smarty->assign('register_article_id', $register_url);
...
');
	...
	if (0 < intval($GLOBALS['_CFG']['sms_code'])) {
		$sms_security_code = $_SESSION['sms_security_code'] = rand(1000, 9999);
		$smarty->assign('sms_security_code', $sms_security_code);
		$smarty->assign('enabled_sms_signin', 1);
	}
	...
	if ($action == 'order_list' || $action == 'order_recycle') {
		...
		$smarty->assign('pay_count', $pay_count);
		$where_confirmed = ' AND oi.order_status ' . db_create_in(array(OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART)) . '  AND oi.shipping_status >= ' . SS_UNSHIPPED . ' AND oi.shipping_status <> ' . SS_RECEIVED . ' AND oi.pay_status ' . db_create_in(array(PS_PAYED, PS_PAYING)) . $where_zc_order;
...
');
			...
			$record_count = $db->getOne($sql);
			$orders = get_user_orders($user_id, $record_count, $page, $is_delete, $where, $order, $show_type);
...
');
				...
				assign_template();
				$page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
				$id = isset($_REQUEST['log_id']) && !empty($_REQUEST['log_id']) ? intval($_REQUEST['log_id']) : 0;
				$log_where = '';

				if ($id) {
					$log_where = ' AND log_id = ' . $id;
				}

				$sql = ' SELECT log_id FROM ' . $GLOBALS['ecs']->table('baitiao_log') . (' WHERE user_id = \'' . $user_id . '\'') . $log_where;
				$log_id = $GLOBALS['db']->getCol($sql);
				$record_count = 0;

				if ($log_id) {
					$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('baitiao_pay_log') . ' WHERE log_id ' . db_create_in($log_id);
					$record_count = $GLOBALS['db']->getOne($sql);
				}

				if ($id) {
					$param = array('act' => $action, 'log_id' => $id);
				}
				else {
					$param = array('act' => $action);
				}

				$pager = get_pager('user.php', $param, $record_count, $page);
