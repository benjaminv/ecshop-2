...
');
	...
	for ($i = 0; $i < $count; $i++) {
		...
		$data .= $order_sn . ',' . $short_order_time . ',' . $consignee . ',' . $total_fee . ',' . $shipping_fee . ',' . $discount . ',' . $coupons . ',' . $integral_money . ',' . $bonus . ',' . $return_amount_price . ',' . $brokerage_amount_price . ',' . $effective_amount_price . ',' . $is_settlement . ',' . $status . '
';
	}
...
function merchants_order_list($page = 0)
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT o.is_frozen,o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.is_delete, o.is_settlement, chargeoff_status,' . '(' . order_amount_field('o.') . ') AS order_total_fee, sob.bill_id, ' . 'o.shipping_time, o.auto_delivery_time, o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id, o.goods_amount, o.shipping_fee, ' . '(' . order_commission_field('o.') . ') AS total_fee, o.discount, o.coupons, o.integral_money, o.bonus, o.user_id, ' . '(' . order_activity_field_add('o.') . ') AS activity_fee ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seller_bill_order') . ' AS sob ' . $where . (' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ');
	...
	for ($i = 0; $i < $count; $i++) {
		...
		if ($row[$i]['is_settlement']) {
			$row[$i]['settlement_status'] = $GLOBALS['_LANG']['settled'];
		}
		else {
			$row[$i]['settlement_status'] = $GLOBALS['_LANG']['not_settled'];
		}
