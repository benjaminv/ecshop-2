		...
		if ($_REQUEST['act'] == 'set_attribute_table' || $_REQUEST['act'] == 'goods_attribute_query') {
			...
			if ($_REQUEST['act'] == 'insert_link_desc' || $_REQUEST['act'] == 'update_link_desc') {
				...
				if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
					...
					$sales_volume_base = (isset($_POST['sales_volume_base']) && !empty($_POST['sales_volume_base']) ? intval($_POST['sales_volume_base']) : 0);
                ...
                WHERE g.goods_id = \'' . $goods_id . '\'');
					...
					check_authz_json('goods_manage');
					$goods_id = intval($_POST['id']);
					$sales_volume_base = intval($_POST['val']);
								
					if ($sales_volume_base == '') {
						make_json_error($_LANG['sales_volume_base_invalid']);
					}
