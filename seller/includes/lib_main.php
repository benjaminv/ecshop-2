...
function return_order_list()
{
	...
	if ($result === false) {
		...
		if (-1 < $filter['store_search']) {
			if ($adminru['ru_id'] == 0) {
				if (0 < $filter['store_search']) {
					...
					if (1 < $filter['store_search']) {
						$where .= ' AND (SELECT count(*) FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' as msi ' . (' WHERE msi.user_id = o.ru_id ' . $store_where . ' LIMIT 1) > 0 ');
					}
				}
				else {
					$store_search = 0;
		...
		if ($store_search == 0 && $adminru['ru_id'] == 0) {
			$where_store = ' AND o.ru_id = 0 AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0';
		}
		else {
			$where_store = '';
		}
		...
		if ($filter['user_name']) {
			$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('order_return') . ' AS o ,' . $GLOBALS['ecs']->table('users') . ' AS u ' . $where . $no_main_order . $where_store;
		}
		else {
...
function get_order_detection_list($is_ajax = 0)
{
	...
	if ($result === false) {
		...
		if ($store_search == 0 && $adminru['ru_id'] == 0) {
			$where_store = ' AND ru_id = 0 ' . ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0';
		}
		else {
			$where_store = '';
		}
