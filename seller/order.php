...
function order_list($page = 0)
{
	...
	if ($result === false) {
		...
		if (!empty($filter['keywords'])) {
			...
			$sql = 'SELECT o.order_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . $leftJoin . $where . $no_main_order . ' GROUP BY o.order_id';
			$record_count = count($GLOBALS['db']->getAll($sql));
			$groupBy = ' GROUP BY o.order_id ';
		}
		else {
			$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . $leftJoin . $where . $no_main_order;
...
{
	$adminru = get_admin_ru_id();
	$where = 'WHERE 1 AND o.ru_id = \'' . $adminru['ru_id'] . '\'';
	$result = get_filter();
	...
	if ($result === false) {
		...
		$leftJion = ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS o ON do.order_id = o.order_id ';
		$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('delivery_order') . ' as do ' . $leftJion . $where;
		...
		$sql = 'SELECT do.delivery_id, do.delivery_sn, do.order_sn, do.order_id, do.add_time, do.action_user, do.consignee, do.country,
                ...
                FROM ' . $GLOBALS['ecs']->table('delivery_order') . ' as do ' . $leftJion . $where . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . '
...
function back_list()
{
	...
	if ($result === false) {
		...
		$leftjoin = ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS o ON bo.order_id = o.order_id ';
		$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('back_order') . ' AS bo ' . $leftjoin . $where;
		$filter['record_count'] = $GLOBALS['db']->getOne($sql);
		$filter['page_count'] = 0 < $filter['record_count'] ? ceil($filter['record_count'] / $filter['page_size']) : 1;
