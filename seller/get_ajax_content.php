...
if ($_REQUEST['act'] == 'get_select_category') {
	...
	if ($type == 1) {
		...
		$sql = 'SELECT g.promote_start_date, g.promote_end_date, g.promote_price, g.sales_volume,g.sales_volume_base, g.goods_name, g.goods_id, g.goods_thumb, g.shop_price, g.market_price, g.original_img FROM ' . $ecs->table('goods') . ' AS g ' . $where . $sort;
	...
	if ($result['moded'] == 'homeFloorFour' || $result['moded'] == 'homeFloorFive' || $result['moded'] == 'homeFloorSeven' || $result['moded'] == 'homeFloorEight' || $result['moded'] == 'homeFloorTen' || $result['moded'] == 'storeOneFloor1' || $result['moded'] == 'storeTwoFloor1' || $result['moded'] == 'storeThreeFloor1' || $result['moded'] == 'storeFourFloor1' || $result['moded'] == 'storeFiveFloor1' || $result['moded'] == 'topicOneFloor' || $result['moded'] == 'topicTwoFloor' || $result['moded'] == 'topicThreeFloor' || $result['moded'] == 'homeFloorSix' && $result['floorMode'] != 1) {
		...
		$sql = 'SELECT g.promote_start_date, g.promote_end_date, g.promote_price, g.sales_volume,g.sales_volume_base, g.goods_name, g.goods_id, g.goods_thumb, g.shop_price, g.market_price, g.original_img  FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $where_goods . ' ORDER BY g.sort_order, g.goods_id DESC ' . $limit;
		$goods_list = $db->getAll($sql);
		...
		if (!empty($goods_list)) {
			foreach ($goods_list as $key => $val) {
				...
				$goods_list[$key]['sales'] = $val['sales_volume']+$val['sales_volume_base'];
