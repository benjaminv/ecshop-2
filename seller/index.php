...
function get_move_sales($day_num)
{
	...
	$where = ' AND o.pay_status = 2';
	$where .= ' AND (SELECT count(*) FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS oi2 WHERE oi2.main_order_id = o.order_id) = 0 ';
	$sql = ' SELECT IFNULL(SUM(' . order_commission_field('o.') . '),0) AS \'ga\', COUNT(o.order_id) AS \'oi\'' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('baitiao_log') . ' AS bai ON o.order_id=bai.order_id WHERE o.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . ' AND o.referer IN(\'touch\', \'mobile\')' . ' AND o.ru_id = \'' . $adminru['ru_id'] . '\'' . $where . ' LIMIT 1';
	$row = $GLOBALS['db']->getRow($sql);
	$arr = array();
	if ($row && $row['oi']) {
		$sales = $row['ga'] / $row['oi'];
...
function get_sub_order($day_num)
{
	...
	$sql = 'SELECT COUNT(o.order_id) AS \'oi\' ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' WHERE o.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . ' AND o.referer NOT IN(\'touch\', \'mobile\')' . ' AND o.ru_id = \'' . $adminru['ru_id'] . '\'' . $where . ' LIMIT 1';
	$row = $GLOBALS['db']->getRow($sql);
	$arr = array();
	if ($row && $row['oi']) {
		$sub_order = $row['oi'];
...
function get_move_sub_order($day_num)
{
	...
	$where = ' AND o.pay_status = 2';
	$where .= ' AND (SELECT count(*) FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS oi2 WHERE oi2.main_order_id = o.order_id) = 0 ';
	$sql = 'SELECT COUNT(*) AS \'oi\' ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' WHERE o.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . ' AND o.referer IN(\'touch\', \'mobile\')' . ' AND o.ru_id = \'' . $adminru['ru_id'] . '\'' . $where . ' LIMIT 1';
	$row = $GLOBALS['db']->getRow($sql);
	$arr = array();
	if ($row && $row['oi']) {
		$sub_order = $row['oi'];
