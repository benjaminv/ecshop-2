...
function return_order_list()
{
	...
	if ($result === false) {
		...
		if ($filter['user_name']) {
			$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('order_return') . ' AS o ,' . $GLOBALS['ecs']->table('order_info') . ' as o, ' . $GLOBALS['ecs']->table('users') . ' AS u ' . $where . $no_main_order . $where_store;
		}
		else {
			$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('order_return') . ' AS r, ' . $GLOBALS['ecs']->table('order_info') . ' as o ' . $where . $no_main_order . ' AND r.order_id = o.order_id';
		}
		...
		if (-1 < $filter['store_search']) {
			if ($adminru['ru_id'] == 0) {
				if (0 < $filter['store_search']) {
					...
					if (1 < $filter['store_search']) {
						$where .= ' AND (SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' as msi ' . (' WHERE msi.user_id = o.ru_id ' . $store_where . ' limit 0, 1) > 0 ');
					}
				}
				else {
					$store_search = 0;
