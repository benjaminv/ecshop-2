		...
		if ($cat['filter_attr']) {
			...
			if (!empty($filter_attr)) {
				...
				foreach ($filter_attr as $k => $v) {
					if (!empty($v)) {
						$attr_sql = 'SELECT attr_value FROM ' . $ecs->table('goods_attr') . (' WHERE goods_attr_id = \'' . $v . '\'');
						$attr_value = $db->getOne($attr_sql);
						$goods_sql = 'SELECT GROUP_CONCAT(goods_id) AS goods_id FROM ' . $ecs->table('goods_attr') . ' WHERE attr_id = \'' . $cat_filter_attr[$k] . ('\' AND attr_value = \'' . $attr_value . '\'');
						$listgoods = $db->getOne($goods_sql);
						$list[] = $listgoods ? explode(',', $listgoods) : array();
					}
				}

				$list = arr_foreach($list);
				$list = $list ? array_unique($list) : array();

				if (!empty($list)) {
					$ext .= ' AND ' . db_create_in($list, 'g.goods_id');
				}
			}
		}
	}
