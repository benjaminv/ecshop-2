...
function bill_detail_list($type = 0, $bill = array())
{
	...
	if ($result === false) {
		if ($type == 1) {
			...
			if (isset($bill['commission_model'])) {
				$filter['commission_model'] = $bill['commission_model'];
			}

			if (isset($bill['proportion'])) {
				$filter['proportion'] = $bill['proportion'];
			}
		}
		else {
		...
		$where = 1;
		$where .= ' AND sbo.seller_id = \'' . $filter['seller_id'] . '\' AND sbo.confirm_take_time > 0';
		$proportion = 0;

		if ($filter['bill_id']) {
			if (!$bill) {
				$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . ' WHERE id = \'' . $filter['bill_id'] . '\' LIMIT 1';
				$bill = $GLOBALS['db']->getRow($sql);
			}

			$where .= ' AND (sbo.confirm_take_time >= \'' . $bill['start_time'] . '\' AND sbo.confirm_take_time <= \'' . $bill['end_time'] . '\')' . ' AND sbo.repair = \'' . $bill['repair_order'] . '\'';
		...
		if ($type == 0) {
			$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('seller_bill_order') . ' AS sbo ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . 'AS o ON o.order_id = sbo.order_id ' . 'WHERE IF(sbo.chargeoff_status > 0, sbo.bill_id = \'' . $filter['bill_id'] . ('\', ' . $where . ')');
		...
		if ($type == 1) {
			if (isset($bill['order_status']) && $bill['order_status'] == OS_RETURNED) {
	...
	$all_should_amount = 0;
	$bill_gain_commission = 0;
	$bill_should_amount = 0;
	$bill_return_amount = 0;
	...
	foreach ($row as $key => $value) {
		...
		if ($type == 1) {
			if ($value['is_settlement'] == 1) {
				...
				$bill_return_amount += $value['return_amount'];
			}
		}
	...
	if ($type == 1) {
		...
		$row['bill_gain_commission'] = $bill_gain_commission;
		$row['bill_should_amount'] = $bill_should_amount;
		$row['bill_return_amount'] = $bill_return_amount;
		$arr = $row;
	}
	else {
...
function commission_bill_list($ajax_bill = 0)
{
	...
	if ($result === false) {
		...
		if (-1 < $filter['bill_apply']) {
			$where .= ' AND b.bill_apply = \'' . $filter['bill_apply'] . '\' AND b.chargeoff_status = 1';
		}
				...
				if ($value['chargeoff_status'] == 1) {
					...
					if ($return_info && (!empty($return_info['return_amount']) || !empty($return_info['return_shippingfee']))) {
						...
						if ($value['return_amount'] && empty($value['return_shippingfee'])) {
							$set_return = 'return_amount = return_amount + \'' . $return_info['return_amount'] . '\'';
						}
						else {
							if ($value['return_shippingfee'] && empty($value['return_amount'])) {
								$set_return = 'return_shippingfee = return_shippingfee + \'' . $return_info['return_shippingfee'] . '\'';
							}
							else {
								if ($value['return_amount'] && $value['return_shippingfee']) {
									$set_return = 'return_amount = return_amount + \'' . $return_info['return_amount'] . '\', return_shippingfee = return_shippingfee + \'' . $return_info['return_shippingfee'] . '\'';
								}
							}
						}
...
function get_bill_amount_detail($bill_id = 0, $seller_id = 0, $proportion = 100, $start_time = 0, $end_time = 0, $chargeoff_status = 0, $commission_model = 0)
{
	...
	$bill_order = get_bill_order_amount($seller_id, $start_time, $end_time, $commission_model, $proportion, $chargeoff_status);
...
function get_bill_order_amount($seller_id, $start_time, $end_time, $commission_model, $proportion, $chargeoff_status, $order_id = 0)
{
...
function merchants_is_settlement($ru_id = 0, $state = '', $filter = array())
{
	...
	for ($i = 0; $i < $count; $i++) {
		...
		if ($bill_id) {
			$sql = 'SELECT commission_model, proportion FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . (' WHERE id = \'' . $bill_id . '\' LIMIT 1');
			$bill = $GLOBALS['db']->getRow($sql);
...
function get_alone_goods_rate($order_id = 0, $ru_id = 0, $order = array())
{
	...
	if ($order) {
		...
		$sql = 'SELECT SUM(sbg.goods_price * sbg.goods_number - sbg.dis_amount) AS total_fee ' . 'FROM ' . $GLOBALS['ecs']->table('seller_bill_goods') . ' AS sbg ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seller_bill_order') . ' AS sbo ON sbo.order_id = sbg.order_id ' . (' WHERE ' . $where . ' AND sbg.commission_rate > 0');
		$res = $GLOBALS['db']->getRow($sql);
	...
	$sql = 'SELECT rec_id, sbg.commission_rate, (sbg.goods_price * sbg.goods_number - sbg.dis_amount) AS goods_amount ' . 'FROM ' . $GLOBALS['ecs']->table('seller_bill_goods') . ' AS sbg ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seller_bill_order') . ' AS sbo ON sbo.order_id = sbg.order_id ' . (' WHERE ' . $where . ' AND sbg.commission_rate > 0');
...
{
	$sql = 'SELECT MIN(start_time) AS min_time, MAX(end_time) AS max_time FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . (' WHERE seller_id = \'' . $seller_id . '\' AND bill_cycle = \'' . $cycle . '\' AND repair_order = 0 LIMIT 1');
	$bill = $GLOBALS['db']->getRow($sql);
	return $bill;
}
...
{
	$sql = 'SELECT id FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . (' WHERE start_time = \'' . $day_start . '\' AND end_time = \'' . $day_end . '\' AND bill_cycle = \'' . $cycle . '\' AND seller_id = \'' . $seller_id . '\' AND repair_order = 0');
