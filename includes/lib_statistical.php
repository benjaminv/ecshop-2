...
function shop_sale_stats($page = 0)
{
	...
	if ($result === false) {
		...
		$leftJoin = '';
		$leftJoin .= ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS o ON o.ru_id = spi.ru_id ';
...
function shop_total_stats()
{
	...
	$leftJoin = '';
	$leftJoin .= ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_return') . ' AS re ON re.order_id = o.order_id ';
	$leftJoin .= ' LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' AS msi ON msi.user_id = o.ru_id ';
