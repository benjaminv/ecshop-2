...
{
	$sql = 'SELECT shop_id, user_id FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' WHERE merchants_audit = 1 LIMIT 15';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
			...
			if (1 < count($desc_img[1])) {
				...
				foreach ($desc_img[1] as $key => $row) {
					if (strpos($row, 'http://') === false && strpos($row, 'https://') === false) {
						...
						if (strpos($row, 'http://images') === false && strpos($row, 'https://images') === false) {
...
function get_baitiao_pay_log_list($log_id = array(), $size = 0, $start = 0)
{
	if (empty($log_id)) {
		return array();
	}
