...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER, $order_type = 0, $deposit_fee = 0)
{
	...
	if ($change_desc && $order_type) {
		...
		if (2 <= count($change_desc_arr)) {
			...
			if ($order_res) {
				if (0 < $order_res['main_order_id']) {
					$sql = 'SELECT order_sn FROM ' . $GLOBALS['ecs']->table('order_info') . 'WHERE order_id = \'' . $order_res['main_order_id'] . '\' LIMIT 1';
					...
					if ($order_res && 0 < $order_res['order_id']) {
						$sql = 'SELECT order_id, order_sn FROM ' . $GLOBALS['ecs']->table('order_info') . ' WHERE main_order_id = \'' . $order_res['order_id'] . '\'';
						$main_order_res = $GLOBALS['db']->getAll($sql);
