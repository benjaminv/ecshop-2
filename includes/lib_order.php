...
function order_goods($order_id)
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		$row['is_return'] = $GLOBALS['db']->getOne('select count(*) from ' . $GLOBALS['ecs']->table('return_goods') . ' where rec_id = \'' . $row['rec_id'] . '\'');
		$goods_list[] = $row;
	}
...
function change_order_goods_storage($order_id, $is_dec = true, $storage = 0, $use_storage = 0, $admin_id = 0, $store_id = 0)
{
	switch ($storage) {
	case 0:
		$sql = 'SELECT goods_id, send_number AS num, extension_code AS extension_code, product_id, warehouse_id, area_id FROM ' . $GLOBALS['ecs']->table('order_goods') . (' WHERE order_id = \'' . $order_id . '\' AND is_real = 1 ');
		break;

	case 1:
	case 2:
		$sql = 'SELECT goods_id, goods_number AS num, extension_code AS extension_code, product_id, warehouse_id, area_id FROM ' . $GLOBALS['ecs']->table('order_goods') . (' WHERE order_id = \'' . $order_id . '\' AND is_real = 1');
		break;
	}
...
{
	if ($type == 1) {
		$res = $GLOBALS['db']->getRow('SELECT o.ru_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . (' WHERE o.order_sn = \'' . $order . '\' LIMIT 1'));
	}
	else {
